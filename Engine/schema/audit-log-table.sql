...
-- agent through the webhook routes or the MCP gateway. Code-execution steps also
-- append one row per sandbox execution (trigger 'code') with outcome details.
--
...
--   detail       -> optional JSON context; for code executions:
--                   {"kind": "code_execution", "step_id", "resource_id",
--                    "outcome": "ok|error|timeout|oom|output_limit|rate_limited|...",
--                    "duration_ms"} — never the code, parameters, output, or secrets
--
